...
LOCAL int sysinit()
{
	...
	kprintf("Entered sysint");
	//Initialize Process Queue in Lock Descriptor
	for(i=0; i< NLOCKS ; i++)
	{
	int j;
	for(j=0; j<NLOCKS; j++)
	{
	 locarr[i].proc_queue[j] = -1;	
	 locarr[i].ltype = -1;
	}
	}

	//Initialize all locks inside a process
	int k;
	for(k=0; k < NPROC ; k++)
	{
	q[k].type = -1;
